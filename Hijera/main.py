...
while running:
    ...
    if Game_Sec >= Game_Limit:
        Game_Schedule.reset()

    if Game_Sec in Game_Schedule:
        for e in Game_Schedule[Game_Sec]:
            enemies.append(e)
    ...
    while makesure:
        ...
        for s in enemies:
            s.attack(SPEED * 3/2, abs(Solon_first.rect.centerx - s.rect.centerx), Solon_first.rect.x)
        for f in eproj:
            f.move(SPEED)
            f.check()
        for l in lightbeams:
